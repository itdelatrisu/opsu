...
public enum GameImage {
	...
	/**
	 * Constructor.
	 * @param filename the image FileHandle name
	 */
	 ...
	 */
	public boolean setSkinImage(FileHandle dir) {
		try {
			...
			// set a new image
			FileHandle file = dir.child(filename);//new FileHandle(dir, filename);
			if (file.exists() && !file.isDirectory() && !Options.isBeatmapSkinIgnored()) {
				skinImage = new Image(file.path());
				return true;
			} else {
				skinImage = null;
				return false;
