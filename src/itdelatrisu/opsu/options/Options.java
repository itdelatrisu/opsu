...
public class Options {
		...
		FULLSCREEN ("graphics.fullscreen", "Fullscreen", false) {
			...
			public void toggle(GameContainer container) {
				...
				if (!getBooleanValue() && !resolution.hasFullscreenDisplayMode()) {
					UI.getNotificationManager().sendBarNotificationFormatted("graphics.fullscreen.resolutionNotApplicable", resolution.toString());
					return;
				}
	...
	public static void setNextFPS(GameContainer container) {
		...
		UI.getNotificationManager().sendBarNotificationFormatted("graphics.framesync", GameOption.TARGET_FPS.getValueString());
	}
	...
	public static void toggleMouseDisabled() {
		...
		UI.getNotificationManager().sendBarNotification((GameOption.DISABLE_MOUSE_BUTTONS.getBooleanValue()) ?
			"mouse.buttons.disabled." : "mouse.buttons.enabled");
	}
