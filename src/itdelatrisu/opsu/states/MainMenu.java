...
public class MainMenu extends BasicGameState {
	...
	public void enter(GameContainer container, StateBasedGame game)
		...
		if (!enterNotification) {
			if (Updater.get().getStatus() == Updater.Status.UPDATE_AVAILABLE) {
				UI.getNotificationManager().sendNotification("update.available", Colors.GREEN);
				enterNotification = true;
			} else if (Updater.get().justUpdated()) {
				String updateMessage = "update.completed";
					...
					UI.getNotificationManager().sendNotification(updateMessage, Colors.GREEN, new NotificationListener() {
						...
						public void click() {
							try {
								...
								UI.getNotificationManager().sendBarNotification("web.noView");
							}
						}
					});
				} else
	...
	public void mousePressed(int button, int x, int y) {
		...
		if (musicPlay.contains(x, y)) {
			if (MusicController.isPlaying()) {
				...
				UI.getNotificationManager().sendBarNotification("menu.playback.pause");
			} else if (!MusicController.isTrackLoading()) {
				MusicController.resume();
				UI.getNotificationManager().sendBarNotification("menu.playback.play");
			}
			return;
		} else if (musicNext.contains(x, y)) {
			nextTrack(true);
			UI.getNotificationManager().sendBarNotification("menu.playback.next");
			return;
		} else if (musicPrevious.contains(x, y)) {
			previousTrack();
			UI.getNotificationManager().sendBarNotification("menu.playback.previous");
			return;
		}
	...
	public void keyPressed(int key, char c) {
		...
		case Input.KEY_C:
			if (MusicController.isPlaying()) {
				...
				MusicController.resume();
				UI.getNotificationManager().sendBarNotification("menu.playback.unpause");
			}
			break;
		case Input.KEY_V:
			nextTrack(true);
	...
	private void enterSongMenu() {
		...
		if (BeatmapSetList.get().getMapSetCount() == 0) {
			((DownloadsMenu) game.getState(Opsu.STATE_DOWNLOADSMENU)).notifyOnLoad("beatmaps.new");
